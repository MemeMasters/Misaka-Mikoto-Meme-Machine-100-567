...
def rnumlistwithoutreplacement(min, max):
    ...
    if checkquota() < 1:
        raise NoQuotaError("Your www.random.org quota has already run out.")
...
def reportquota():
    ...
    opener = urllib.request.build_opener()
    quota = opener.open(request).read()
    print("This IP address has {} bits left. Visit http://www.random.org/quota for more information.".format(quota))
    return quota
